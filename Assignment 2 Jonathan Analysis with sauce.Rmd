...
BiocManager::install(c("biomaRt", "DESeq2", "EDASeq", "M3C", "EnhancedVolcano", "ComplexHeatmap"))
...
healthy_cols <- paste0("HEALTHY ", healthy_patients)
healthy_vals <- diffex_logs[, ..healthy_cols]
healthy_vals$Median <- apply(healthy_vals, MARGIN=1, FUN = median, na.rm=TRUE)
ast_cols <- paste0("AST ", ast_patients)
ast_vals <- diffex_logs[, ..ast_cols]
ast_vals$Median <- apply(ast_vals, MARGIN=1, FUN = median, na.rm=TRUE)
heatmap_df <- t(heatmap_df)
ha1 <- HeatmapAnnotation("healthy (Median)" = healthy_vals$Median, "ast (Median)" = ast_vals$Median)
Heatmap(heatmap_df, name = "Log2 Normalized Expression", column_title = "Expression of Differentially Expressed Genes", top_annotation = ha1)
```
Part 5:
```{r}
