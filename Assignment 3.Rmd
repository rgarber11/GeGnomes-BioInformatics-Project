...
ggplot(lodes_data, aes(y = `Sample Frequency`, x = `Amount of Genes Used`, stratum = `Cluster`, alluvium = alluvium, fill = `Cluster`, label = `Cluster`)) + scale_fill_brewer(type = "qual", palette = "Set3") + geom_flow(stat = "alluvium", lode.guidance = "frontback", color = "darkgrey") + geom_stratum() +  scale_y_continuous(name = "Sample Frequency", limits =c(0, 57)) + ggtitle("ContinuousClusterPlus Sample Clustering Based on Number of Genes")
...
ha <- rowAnnotation(Condition = as.data.frame(metadata) %>% select(c("Patient_id","condition")) %>% tibble::deframe(), PAM = as.character(fivek_pamResults$clustering), KMeans = as.character(kclustInfo %>% select(c("samples", "5000")) %>% tibble::deframe()),  CCC = as.character(results_5000[[10]]$consensusClass), col = list(Condition = cols_condition, PAM = cols_2, KMeans = cols_2, CCC = cols_10), show_legend = c(KMeans = FALSE), annotation_legend_param = list(PAM = list(title = "k = 2"), CCC = list(title = "k = 10", labels = as.character(1:10))))
