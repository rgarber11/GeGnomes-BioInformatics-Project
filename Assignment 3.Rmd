...
row.names(t_fiveK) = colnames(fivek_log_tpm_counts)
t_fiveK <- t_fiveK[-1] #Original included hgnc_symbol in group
t_fiveK <- t_fiveK %>% mutate(across(everything(), as.numeric)) #Original turned 0s into characters which messed with pam
...
t_ten <- t_ten[-1]
t_ten <- t_ten %>% mutate(across(everything(), as.numeric))
...
t_hundred <- t_hundred %>% mutate(across(everything(), as.numeric))
...
t_onek <- t_onek[-1]
t_onek <- t_onek %>% mutate(across(everything(), as.numeric))
...
t_tenk <- t_tenk[-1]
t_tenk <- t_tenk %>% mutate(across(everything(), as.numeric))
...
ha <- rowAnnotation(Condition = as.data.frame(metadata) %>% select(c("Patient_id","condition")) %>% tibble::deframe(), PAM = as.character(fivek_pamResults$clustering), KMeans = as.character(kclustInfo %>% select(c("samples", "fivek")) %>% tibble::deframe()),  CCC = as.character(results_5000[[10]]$consensusClass), col = list(Condition = cols_condition, PAM = cols_2, KMeans = cols_2, CCC = cols_10), show_legend = c(KMeans = FALSE), annotation_legend_param = list(PAM = list(title = "k = 2"), CCC = list(title = "k = 10", labels = as.character(1:10))))
...
#p.adjust(summaryConditionPAM$p.value, n=6) # dependent (p low reject Ho) n = 3 as there are 6 different hypotheses (condition v PAM, condition v KMeans, condition v CCC, PAM v KMeans, PAM v CCC, KMeans v CCC)
statTestResults <- list()
statTestResults$ConditionPAM = c("p-value"=summaryConditionPAM$p.value, "p-adj"=p.adjust(summaryConditionPAM$p.value, n=6))
...
statTestResults$ConditionKMEANS = c("p-value"=summaryConditionKMEANS$p.value, "p-adj"=p.adjust(summaryConditionKMEANS$p.value, n=6))
chiSqData$CCC <- results_5000[[10]]$consensusClass
summaryConditionCCC <- chisq.test(table(chiSqData$condition, chiSqData$CCC))
statTestResults$ConditionCCC <- c("p-value"=summaryConditionCCC$p.value, "p-adj"=p.adjust(summaryConditionCCC$p.value, n=6))
summaryPAMKMeans <- chisq.test(table(chiSqData$PAM, chiSqData$KMeans))
statTestResults$PAMKMeans <- c("p-value"=summaryPAMKMeans$p.value, "p-adj"=p.adjust(summaryPAMKMeans$p.value, n=6))
summaryPAMCCC <- chisq.test(table(chiSqData$PAM, chiSqData$CCC))
statTestResults$PAMCCC <- c("p-value"=summaryPAMCCC$p.value, "p-adj"=p.adjust(summaryPAMCCC$p.value, n=6))
summaryKMeansCCC <- chisq.test(table(chiSqData$KMeans, chiSqData$CCC))
statTestResults$KMeansCCC <- c("p-value"=summaryKMeansCCC$p.value, "p-adj"=p.adjust(summaryKMeansCCC$p.value, n=6))
statTestResults <- as.data.frame(statTestResults)
statTestResults
chiSqData <- chiSqData %>% mutate(across(c(3:5), as.character))
chiSqData$CCC <- paste0("\U00A0", chiSqData$CCC) #Put columns in correct order
chiSqData$CCC[chiSqData$CCC == paste0("\U00A0","10")] <- "10" #Not super efficient, but works
pm <- ggpairs(chiSqData, mapping = aes(color = condition), title = "Comparison of PAM, KMeans, CCC and experimental clustering", columns = 2:5, lower = list(
  discrete = "autopoint"
), diag = list(
...
```{r}
metaCondensed_Vector <- metaCondensed %>% tibble::deframe()
top_5000_counts_data_healthy <- top_5000_counts_data[, metaCondensed_Vector[colnames(top_5000_counts_data)] == "healthy"]
