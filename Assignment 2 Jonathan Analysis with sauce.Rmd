...
ast_cols <- names(diffex_logs[, ..ast_patients])
setnames(diffex_logs, ast_cols, paste0("AST ", ast_cols))
healthy_patients <- metadata[condition == "healthy"]$Patient_id
healthy_cols <- names(diffex_logs[, ..healthy_patients])
setnames(diffex_logs, healthy_cols, paste0("HEALTHY ", healthy_cols))
heatmap_df <- as.data.frame(diffex_logs) %>% tibble::column_to_rownames("hgnc_symbol")
heatmap_df <- t(heatmap_df)
col_fun = colorRamp2(c(0, 5, 10), c("blue", "white", "red"))
Heatmap(heatmap_df, name = "Log2 Normalized Expression", col = col_fun, column_title = "Expression of Differentially Expressed Genes")
```
Part 5:
```{r}
